...
class ShutterButtonViewController: UIViewController {
    ...
    private(set) lazy var innerImageView: UIImageView = {
...
//        self.view.layer.borderWidth = 2
        ...
        innerCircle.addSubview(innerImageView)
...
//                    guard let editedData = editedImage.jpegData(compressionQuality: 1.0) else { return }
                    guard let editedData = image.jpegData(compressionQuality: 1.0) else { return }
//                    DataEngine.shared.save(imageData: editedData)
                    let topUrl = URL(string: self.cloud?.topImage ?? "")
                    let topImage = try! UIImage(withContentsOfUrl: topUrl!)
                    let bottomUrl = URL(string: self.cloud?.bottomImage ?? "")
                    let bottomImage = try! UIImage(withContentsOfUrl: bottomUrl!)
                    DataEngine.shared.save(imageData: editedData, topImage: topImage!, bottomImage: bottomImage!)
